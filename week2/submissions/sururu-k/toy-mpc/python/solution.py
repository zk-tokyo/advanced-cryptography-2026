...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    # 先頭 n-1 個は与えられた乱数そのもの。最後の1個で辻褄を合わせる。
    head = [value % modulus for value in randomness]
    last = (secret - sum(head)) % modulus
    return head + [last]
...
def add_shares(
    ...
    # 加算は通信なしで済む（各partyが自分のshareを足すだけ）。
    return [
        (left + right) % modulus
...
def beaver_multiply(
    ...
    # 定数項 de は公開値。全員が足すと party 数だけ重複するので party 0 だけ。
...
def ot_receiver_request(
    ...
    if choice == 0:
        return masked
    # senderからはg^bとA*g^bの区別がつかない（bが一様なのでどちらも一様）。
    return (sender_public * masked) % OT_P
...
def ot_sender_encrypt(
    ...
    # branch 0: B^a、branch 1: (B/A)^a。receiverが選ばなかった方は
    # g^{ab}にならないので、鍵を作れない。
    shared_0 = pow(request, sender_secret, OT_P)
    inverse_public = pow(sender_public, OT_P - 2, OT_P)
...
def ot_receiver_decrypt(
    ...
    # choiceがどちらでも、自分が復号できる側の鍵はA^b = g^{ab}になる。
...
def gmw_and(
    ...
    # xy = x0y0 ^ x1y1 ^ x0y1 ^ x1y0。前2つはlocalに計算でき、
    # 残るクロス項2つをOTで分散させる。
    # session 01: P0がsender。P1はy1を選ぶので r01 ^ (x0 & y1) を受け取る。
    cross_01 = _ot_transfer_bit(r01, r01 ^ x0, y1, *ot_secrets[0])
    # session 10: 役割を入れ替えて同じことをする。
    cross_10 = _ot_transfer_bit(r10, r10 ^ x1, y0, *ot_secrets[1])

    # senderは自分のマスクを、receiverは受け取った値を持ち分とする。
