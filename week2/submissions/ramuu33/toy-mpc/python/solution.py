...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    if not randomness:
        raise ValueError("randomness must contain at least one value")

    #有限体の値に直す
    shares = [value % modulus for value in randomness]
    #全シェアの合計が secret になるようにする。
    final_share = (secret - sum(shares)) % modulus
    return shares + [final_share]
...
def add_shares(
    ...
    # (x0 + x1 + ...) + (y0 + y1 + ...) と同じになるよう、同じpartyの分だけ足す。
    return [
        (left + right) % modulus
...
def beaver_multiply(
    ...
    for index in range(party_count):
        #partyごとのシェアとして作る
        value = c_shares[index] + d * b_shares[index] + e * a_shares[index]
        if index == 0:
            #二重加算を避けるため
            value += d * e
...
def ot_receiver_request(
    ...
    #choice=0なら B=g^b、choice=1なら B=A*g^b にする
...
def ot_sender_encrypt(
    ...
    branch_1_request = (request * sender_public_inverse) % OT_P

    #requestがBのときはB^a、A*g^bのときは(B/A)^a　それぞれの枝の鍵のイメージ
    shared_0 = pow(request, sender_secret, OT_P)
    shared_1 = pow(branch_1_request, sender_secret, OT_P)
    #XORで暗号化
...
def ot_receiver_decrypt(
    ...
    #receiverが選んだ枝では、sender側の鍵と同じ A^b = g^(ab) を計算
...
def gmw_and(
    ...
    y0, y1 = y_shares
    r01, r10 = masks
    (sender_secret_01, receiver_secret_01), (
        sender_secret_10,
        receiver_secret_10,
    ) = ot_secrets

    #r01または r01 XOR x0を受け取る
    #r01 XOR(x0 & y1)に等しい
    cross_01 = _ot_transfer_bit(
        r01,
        r01 ^ x0,
        y1,
        sender_secret_01,
    ...
    #r10またはr10 XOR x1を受け取る
    #r10 XOR (x1 & y0) に等しい。
    cross_10 = _ot_transfer_bit(
        r10,
        r10 ^ x1,
        y0,
        sender_secret_10,
    ...
    #展開した4項を、2人のXORシェアに分ける。
    return (x0 & y0) ^ r01 ^ cross_10, (x1 & y1) ^ r10 ^ cross_01
