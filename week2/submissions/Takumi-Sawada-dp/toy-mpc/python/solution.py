...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    if not randomness:
        raise ValueError()
    r_canonical = [r % modulus for r in randomness]
    r_sum = sum(randomness) % modulus
    last_s = (secret - r_sum) % modulus
    return r_canonical + [last_s]
...
def beaver_multiply(
    ...
    d = reconstruct([(x_s - a_s) % modulus for x_s, a_s in zip(x_shares, a_shares)], modulus)
    e = reconstruct([(y_s - b_s) % modulus for y_s, b_s in zip(y_shares, b_shares)], modulus)

    xy_shares = [(c_s + (d * b_s) + (e * a_s)) % modulus for a_s, b_s, c_s in zip(a_shares, b_shares, c_shares)]
    xy_shares[0] = (xy_shares[0] + d * e) % modulus
...
def ot_receiver_request(
    ...
    return (A * B) % OT_P if choice else B
...
def ot_sender_encrypt(
    ...
    B = request
    k_0 = pow(B, a, OT_P)
    k_1 = pow(B * pow(A, -1, OT_P) % OT_P, sender_secret, OT_P)

    return (
        xor_bytes(message_0, derive_pad(k_0, 0, len(message_0))),
        xor_bytes(message_1, derive_pad(k_1, 1, len(message_1))),
    )
...
def ot_receiver_decrypt(
    ...
    r = receiver_secret
    b = choice
    c = ciphertexts[1] if b else ciphertexts[0]
    k = derive_pad(pow(A, r, OT_P), b, len(c))
    return xor_bytes(c, k)
...
def gmw_and(
    ...
    """
    _validate_bit_shares(x_shares, "")
    _validate_bit_shares(y_shares, "")
    _validate_bit_shares(masks, "")
