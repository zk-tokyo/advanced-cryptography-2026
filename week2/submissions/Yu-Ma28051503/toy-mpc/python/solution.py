...
def add_shares(
    ...
    if (len(left_shares) < 1) or (len(right_shares) < 1):
        raise ValueError
    ...
    return [
        (lefts + rights) % modulus
        for lefts, rights in zip(left_shares, right_shares)
    ]
...
def beaver_multiply(
    ...
    shares = [(c + d*b + e*a) % modulus
            for a, b, c in zip(triple[0], triple[1], triple[2])
    ]

    shares[0] = (shares[0] + d*e) % modulus
...
def ot_sender_encrypt(
    ...
    validate_group_element(request)
    if len(message_0) != len(message_1):
        raise ValueError

    key_0 = pow(request, sender_secret, OT_P)
    key_1 = pow(request * pow(ot_sender_setup(sender_secret), OT_P-2, OT_P), sender_secret, OT_P)

    pad_0 = derive_pad(key_0, 0, len(message_0))
    pad_1 = derive_pad(key_1, 1, len(message_1))

    encrypted_0 = xor_bytes(message_0, pad_0)
    encrypted_1 = xor_bytes(message_1, pad_1)

    return (encrypted_0, encrypted_1)
...
def ot_receiver_decrypt(
    ...
    key = pow(sender_public, receiver_secret, OT_P)
    pad = derive_pad(key, choice, len(ciphertexts[choice]))
    plaintext = xor_bytes(ciphertexts[choice], pad)
...
def gmw_and(
    ...
    # x AND y = (x0 XOR x1) AND (y0 XOR y1)
    #         = (x0 AND y0) XOR (x0 AND y1) XOR (x1 AND y0) XOR (x1 AND y1)
    # x0 AND y0: offline
    # x1 AND y1: offline
    # x0 AND y1: Session01
    # x1 AND y0: Session10
    ...
    received_01 = _ot_transfer_bit(r01, r01 ^ x0, y1, ot_secrets[0][0], ot_secrets[0][1])
    received_10 = _ot_transfer_bit(r10, r10 ^ x1, y0, ot_secrets[1][0], ot_secrets[1][1])

    out0 = (x0 & y0) ^ r01 ^ received_10
    out1 = (x1 & y1) ^ r10 ^ received_01
