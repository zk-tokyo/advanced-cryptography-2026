...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    if len(randomness) < 1:
        raise ValueError("share requires at least two parties")
    shares = [value % modulus for value in randomness]
    # 最後のpartyには、全シェアの合計がsecretに戻るように帳尻を合わせた値を渡す。
...
def add_shares(
    ...
    # 各partyが自分のシェア同士を足すだけでよい。復元(reconstruct)は不要。
    return [
        (left + right) % modulus
...
def ot_receiver_request(
    ...
    if choice == 1:
        # choice=1のときだけAを掛ける。bを0..q-1一様に選ぶことで、senderからは
        # choiceによらずrequestの分布が区別できなくなる。
...
def ot_sender_encrypt(
    ...
    if len(message_0) != len(message_1):
        raise ValueError("message_0 and message_1 must have the same length")

    sender_public = pow(OT_G, sender_secret, OT_P)
    inverse_public = pow(sender_public, -1, OT_P)

    # senderはreceiverのchoiceを知らないため、両方の枝の鍵を計算しておく。
    # requestがchoice=0由来ならshared_0が、choice=1由来ならshared_1が、
    # receiverが後で計算するA^bと一致する。
...
def ot_receiver_decrypt(
    ...
    # receiverは自分のchoiceと一致する枝の鍵しか計算できない＝それしか復号できない。
...
def gmw_and(
    ...
    if len(ot_secrets) != 2:
        raise ValueError("ot_secrets must contain one pair per OT session")

    # x∧y = (x0⊕x1)∧(y0⊕y1) = x0y0 ⊕ x0y1 ⊕ x1y0 ⊕ x1y1。
    # x0y0はP0が、x1y1はP1がローカルに計算できる。x0y1とx1y0はpartyをまたぐので
    # OTでXORシェアにする。
    x0, x1 = x_shares
    y0, y1 = y_shares
    r01, r10 = masks
    (secret_01_sender, secret_01_receiver), (secret_10_sender, secret_10_receiver) = (
        ot_secrets
    )

    # セッション01: P0がsender(x0を隠す)、P1がy1でchoiceして x0∧y1 のシェアを得る。
    t1 = _ot_transfer_bit(
        r01, r01 ^ x0, y1, secret_01_sender, secret_01_receiver
    )
    # セッション10: P1がsender(x1を隠す)、P0がy0でchoiceして x1∧y0 のシェアを得る。
    t0 = _ot_transfer_bit(
        r10, r10 ^ x1, y0, secret_10_sender, secret_10_receiver
    )
