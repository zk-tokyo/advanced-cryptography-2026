...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    random_shares = [value % modulus for value in randomness]
    final_share = (secret - sum(random_shares)) % modulus
...
def beaver_multiply(
    ...
    result = [
        (c_shares[index] + d * b_shares[index] + e * a_shares[index])
        % modulus
...
def ot_sender_encrypt(
    ...
    branch_0_shared = pow(request, sender_secret, OT_P)
    request_without_public = request * pow(sender_public, -1, OT_P) % OT_P
    branch_1_shared = pow(request_without_public, sender_secret, OT_P)

    pad_0 = derive_pad(branch_0_shared, 0, len(message_0))
    pad_1 = derive_pad(branch_1_shared, 1, len(message_1))
...
def gmw_and(
    ...
    cross_01_share = _ot_transfer_bit(
        r_01,
        r_01 ^ x_0,
        y_1,
        sender_01,
    ...
    cross_10_share = _ot_transfer_bit(
        r_10,
        r_10 ^ x_1,
        y_0,
        sender_10,
    ...
    return (
        (x_0 & y_0) ^ r_01 ^ cross_10_share,
        (x_1 & y_1) ^ cross_01_share ^ r_10,
    )
