...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    return [rand % modulus for rand in randomness] + [last]
...
def add_shares(
    ...
    return [(l + r) % modulus for l, r in zip(left_shares, right_shares)]
...
def ot_receiver_request(
    ...
    return sender_public * pow(OT_G, receiver_secret, OT_P) % OT_P if choice else pow(OT_G, receiver_secret, OT_P)
...
def ot_sender_encrypt(
    ...
    validate_sender_scalar(sender_secret, "sender_secret")
    branch_0 = pow(request, sender_secret, OT_P)
    branch_1 = pow(request * pow(ot_sender_setup(sender_secret), -1, OT_P), sender_secret, OT_P)
    return (
        xor_bytes(message_0, derive_pad(branch_0, 0, len(message_0))),
        xor_bytes(message_1, derive_pad(branch_1, 1, len(message_1))),
    )
...
def ot_receiver_decrypt(
    ...
    return xor_bytes(ciphertexts[choice], derive_pad(shared, choice, len(ciphertexts[choice])))
...
def gmw_and(
    ...
    if len(x_shares) != 2 or len(y_shares) != 2:
        raise ValueError("x_shares and y_shares must contain exactly two shares")
    if len(ot_secrets) != 2:
        raise ValueError("ot_secrets must contain two sessions")
    session_01 = _ot_transfer_bit(
        masks[0],
        masks[0] ^ x_shares[0],
        y_shares[1],
        ot_secrets[0][0],
    ...
    # z0 XOR z1 = (x0 XOR x1) AND (y0 XOR y1)
    # (x0 AND y1) ^ (x0 AND y0) ^ (x1 AND y0) ^ (x1 AND y1) = (x0 XOR x1) AND (y0 XOR y1)
    return (session_01 ^ masks[0] ^ (x_shares[0] & y_shares[0]),
            session_10 ^ masks[1] ^ (x_shares[1] & y_shares[1]))
