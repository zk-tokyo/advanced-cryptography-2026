...
def ot_receiver_decrypt(
    ...
    pad = derive_pad(
        shared,
        choice,
        len(selected_ciphertext),
    )

    # ciphertext XOR pad = message
    return xor_bytes(selected_ciphertext, pad)
...
def gmw_and(
    ...
    session_01, session_10 = ot_secrets

    if len(session_01) != 2 or len(session_10) != 2:
        raise ValueError(
            "each OT session must contain sender and receiver secrets"
        )
    ...
    sender_secret_01, receiver_secret_01 = session_01
    sender_secret_10, receiver_secret_10 = session_10

    # P0がsender、P1がreceiver
    # 結果: received_01 = r01 XOR (x0 AND y1)
    ...
    # P1がsender、P0がreceiver
    # 結果: received_10 = r10 XOR (x1 AND y0)
