...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    share_last = (secret - sum(randomness)) % modulus
    shares = ([value % modulus for value in randomness] + [share_last])
    return shares
...
def beaver_multiply(
    ...
    result = add_shares(c_shares, scale_shares(b_shares, d, modulus), modulus)
    result = add_shares(result, scale_shares(a_shares, e, modulus), modulus)
...
def ot_sender_encrypt(
    ...
    A = pow(OT_G, sender_secret, OT_P)
    k0 = pow(request, sender_secret, OT_P)
    k1 = pow((request * pow(A, -1, OT_P)) % OT_P, sender_secret, OT_P)
    pad0 = derive_pad(k0, 0, len(message_0))
    pad1 = derive_pad(k1, 1, len(message_1))
    c0 = xor_bytes(message_0, pad0)
    c1 = xor_bytes(message_1, pad1)
    return (c0, c1)
...
def ot_receiver_decrypt(
    ...
    ct = ciphertexts[choice]
    pad = derive_pad(key, choice, len(ct))
    return xor_bytes(ct, pad)
...
def gmw_and(
    ...
    y0, y1 = y_shares
    r01, r10 = masks
    (s01, rc01), (s10, rc10) = ot_secrets

    # session 01: P0 が Sender, message = (r01, r01 ^ x0), P1 が y1 で choose
    t01 = _ot_transfer_bit(r01, r01 ^ x0, y1, s01, rc01)
    # session 10: P1 が Sender, message = (r10, r10 ^ x1), P0 が y0 で choose
    t10 = _ot_transfer_bit(r10, r10 ^ x1, y0, s10, rc10)
