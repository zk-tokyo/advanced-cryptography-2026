...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    last_share = (secret - sum(randomness)) % modulus
    return [value % modulus for value in randomness] + [last_share]
...
def beaver_multiply(
    ...
    term1 = scale_shares(triple[1], d, modulus)
    term2 = scale_shares(triple[0], e, modulus)
    result = add_shares(triple[2], term1, modulus)
    result = add_shares(result, term2, modulus)
...
def ot_receiver_request(
    ...
    g_to_b = pow(OT_G, receiver_secret, OT_P)
    if choice == 0:
        return g_to_b
    return (sender_public * g_to_b) % OT_P
...
def ot_sender_encrypt(
    ...
    a = sender_secret
    A = pow(OT_G, a, OT_P)
    shared0 = pow(request, a, OT_P)
    shared1 = pow(request * pow(A, -1, OT_P), a, OT_P)
    pad0 = derive_pad(shared0, 0, len(message_0))
    pad1 = derive_pad(shared1, 1, len(message_1))
...
def gmw_and(
    ...
    # セッション01: P0が (r01, r01^x0) を用意、P1がy1で選ぶ
    cross_01 = _ot_transfer_bit(r01, r01 ^ x0, y1, sender_01, receiver_01)

    # セッション10: P1が (r10, r10^x1) を用意、P0がy0で選ぶ
    cross_10 = _ot_transfer_bit(r10, r10 ^ x1, y0, sender_10, receiver_10)

    z0 = (x0 & y0) ^ r01 ^ cross_10
    z1 = (x1 & y1) ^ cross_01 ^ r10
