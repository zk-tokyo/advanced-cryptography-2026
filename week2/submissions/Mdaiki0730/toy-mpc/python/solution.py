...
def beaver_multiply(
    ...
    result = []
    for index, (c, b, a) in enumerate(zip(c_shares, b_shares, a_shares)):
        value = (c + d * b + e * a) % modulus
        if index == 0:
            value = (value + d * e) % modulus
        result.append(value)
    return result
...
def ot_sender_encrypt(
    ...
    inverse_sender_public = pow(sender_public, -1, OT_P)

    shared_0 = pow(request, sender_secret, OT_P)
    branch_1_base = (request * inverse_sender_public) % OT_P
    shared_1 = pow(branch_1_base, sender_secret, OT_P)

    pad_0 = derive_pad(shared_0, 0, length)
    pad_1 = derive_pad(shared_1, 1, length)
...
def gmw_and(
    ...
    if len(ot_secrets) != 2:
        raise ValueError("ot_secrets must contain exactly two entries")
    ...
    # Session 01: P1 learns r01 XOR (x0 & y1) by choosing on y1.
    t1 = _ot_transfer_bit(r01, r01 ^ x0, y1, sender_01, receiver_01)
    # Session 10: P0 learns r10 XOR (x1 & y0) by choosing on y0.
    s0 = _ot_transfer_bit(r10, r10 ^ x1, y0, sender_10, receiver_10)
