...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    """
    if not randomness or len(randomness) < 1:
        raise ValueError("at least one random share is required")
...
def beaver_multiply(
    ...
    """
    _validate_same_share_count(x_shares, y_shares, *triple)
    _validate_modulus(modulus)
    d = reconstruct(sub_shares(x_shares, triple[0], modulus), modulus)
    e = reconstruct(sub_shares(y_shares, triple[1], modulus), modulus)
    xys = add_shares(triple[2], scale_shares(triple[1], d, modulus), modulus)
    xys = add_shares(xys, scale_shares(triple[0], e, modulus), modulus)
    xys[0] = (xys[0] + d * e) % modulus
    return xys
...
def ot_receiver_request(
    ...
    """
    _validate_bit(choice, "choice")
    validate_receiver_scalar(receiver_secret, "receiver_secret")
    B = pow(OT_G, receiver_secret, OT_P) % OT_P
    if choice == 0:
        return B
    else:
        return (sender_public * B) % OT_P
...
def ot_sender_encrypt(
    ...
    shared_0 = pow(B, a, OT_P)
    A_inv = pow(A, -1, OT_P)
    shared_1 = pow((B * A_inv) % OT_P, a, OT_P)
...
def ot_receiver_decrypt(
    ...
    K = derive_pad(pow(sender_public, receiver_secret, OT_P), choice, len(ciphertexts[choice]))
    return xor_bytes(ciphertexts[choice], K)
...
def gmw_and(
    ...
    if any(len(s) != 2 for s in ot_secrets):
        raise ValueError("ot_secrets must contain two (sender_secret, receiver_secret) pairs")
    # use mask01 for 0 to 1, and mask10 for 1 to 0
    r01, r10 = masks
    # session 01: P0 sends (r01, r01 XOR x0), P1 chooses y1
    x0, y1 = x_shares[0], y_shares[1]
    sender_secret_01, receiver_secret_01 = ot_secrets[0]
    recv01 = _ot_transfer_bit(r01, r01 ^ x0, y1, sender_secret_01, receiver_secret_01)
    
    # session 10: P1 sends (r10, r10 XOR x1), P0 chooses y0
    x1, y0 = x_shares[1], y_shares[0]
    sender_secret_10, receiver_secret_10 = ot_secrets[1]
    recv10 = _ot_transfer_bit(r10, r10 ^ x1, y0, sender_secret_10, receiver_secret_10)
