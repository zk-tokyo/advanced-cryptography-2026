...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    if len(randomness) < 1:
        raise ValueError("a share must contain at least two parties")
    ret = [r % modulus for r in randomness]
    ret.append((secret - sum(ret))%modulus)
    return ret
...
def add_shares(
    ...
    return [
            (l+r)%modulus
            for l,r in zip(left_shares, right_shares)
    ]
...
def beaver_multiply(
    ...
    _validate_same_share_count(x_shares, triple[0])
    _validate_same_share_count(x_shares, triple[1])
    _validate_same_share_count(x_shares, triple[2])
    ...
    for i in range(len(x_shares)):
        ...
        value = c + d*b + e*a
        if i==0:
            ret.append((value+d*e)%modulus)
        else:
            ret.append(value%modulus)
    return ret
...
def ot_receiver_request(
    ...
    b_point = pow(OT_G, receiver_secret, OT_P)
    if choice == 0:
        return b_point
    return (sender_public * b_point) % OT_P
...
def ot_sender_encrypt(
    ...
    inv_public = pow(sender_public, -1, OT_P)
    key_0 = pow(request, sender_secret, OT_P)
    key_1 = pow((request * inv_public) % OT_P, sender_secret, OT_P)
...
def gmw_and(
    ...
    x0, x1 = x_shares
    y0, y1 = y_shares
    recv_01 = _ot_transfer_bit(r01, r01 ^ x0, y1, ot_secrets[0][0], ot_secrets[0][1])
    recv_10 = _ot_transfer_bit(r10, r10 ^ x1, y0, ot_secrets[1][0], ot_secrets[1][1])
    z0 = (x0 & y0) ^ r01 ^ recv_10
    z1 = (x1 & y1) ^ recv_01 ^ r10
    return z0, z1
