...
def share(secret: int, randomness: list[int], modulus: int) -> ShareVector:
    ...
    total = 0
    shares = []
    for random_num in randomness:
        share = random_num % modulus
        total += share
        shares.append(share)

    final_share = (secret - total) % modulus
...
def beaver_multiply(
    ...
    _validate_same_share_count(x_shares, a_shares)
    _validate_same_share_count(x_shares, b_shares)
    _validate_same_share_count(x_shares, c_shares)
        
    d = reconstruct(sub_shares(x_shares, a_shares, modulus), modulus)
    e = reconstruct(sub_shares(y_shares, b_shares, modulus), modulus)

    shares = []
    for i, (a_i, b_i, c_i) in enumerate(zip(a_shares, b_shares, c_shares)):
        de = d * e if i == 0 else 0
        share = c_i + d * b_i + e * a_i + de
        shares.append(share % modulus)
    return shares
...
def ot_receiver_request(
    ...
    receiver_request = g_b if choice == 0 else g_b * sender_public % OT_P
...
def ot_sender_encrypt(
    ...
    """
    k0 = pow(request, sender_secret, OT_P)
    A_inv = pow(ot_sender_setup(sender_secret), -1, OT_P)
    k1 = pow(request * A_inv, sender_secret, OT_P)
    return (
        xor_bytes(message_0, derive_pad(k0, 0, len(message_0))),
        xor_bytes(message_1, derive_pad(k1, 1, len(message_1)))
    )
...
def ot_receiver_decrypt(
    ...
    validate_choice(choice)
    validate_receiver_scalar(receiver_secret)
    key = derive_pad(pow(sender_public, receiver_secret, OT_P), choice, len(ciphertexts[choice]))
    return xor_bytes(ciphertexts[choice], key)
...
def gmw_and(
    ...
    y0, y1 = y_shares
    r01, r10 = masks
    sec_01, sec_10 = ot_secrets
    ot_01 = _ot_transfer_bit(r01, r01 ^ x0, y1, sec_01[0], sec_01[1])
    ot_10 = _ot_transfer_bit(r10, r10 ^ x1, y0, sec_10[0], sec_10[1])
    return (
        x0 & y0 ^ r01 ^ ot_10,
        x1 & y1 ^ r10 ^ ot_01
    )