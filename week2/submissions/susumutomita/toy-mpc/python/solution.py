...
def ot_sender_encrypt(
    ...
    shifted = (request * pow(sender_public, -1, OT_P)) % OT_P
    shared_1 = pow(shifted, sender_secret, OT_P)
...
def gmw_and(
    ...
    # Session 01: P0 is the sender, P1 picks with its share y1 and learns
    # r01 XOR (x0 AND y1) without revealing y1 to P0.
    t01 = _ot_transfer_bit(
        r01,
        r01 ^ x0,
        y1,
        ot_secrets[0][0],
    ...
    # Session 10: roles swap, P0 learns r10 XOR (x1 AND y0).
    t10 = _ot_transfer_bit(
        r10,
        r10 ^ x1,
        y0,
        ot_secrets[1][0],
