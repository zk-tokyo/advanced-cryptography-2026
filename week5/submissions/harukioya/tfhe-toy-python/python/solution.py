...
def decrypt_lwe(
    ...
    plaintext_with_noise = normalize(ciphertext.b - inner, params.q)
    rounded_plaintext = (plaintext_with_noise + params.delta // 2) // params.delta
...
def lwe_scalar_mul(
    ...
    new_a: list[int] = []
    for value in ciphertext.a:
        product = normalize(scalar * value, params.q)
        new_a.append(product)
...
def external_product(
    ...
    result_a = zero_poly(params)
    result_b = zero_poly(params)
    
    for j in range(params.l):
        digit_poly_a = decomposed_a[j]
        digit_poly_b = decomposed_b[j]
        
        row_for_a = control.rows_for_a[j]
        row_for_b = control.rows_for_b[j]
        
        term_from_a = rlwe_plain_mul(row_for_a, digit_poly_a, params)
        term_from_b = rlwe_plain_mul(row_for_b, digit_poly_b, params)
        
        result_a = poly_add(result_a, term_from_a.a, params)
        result_b = poly_add(result_b, term_from_a.b, params)
        result_a = poly_add(result_a, term_from_b.a, params)
        result_b = poly_add(result_b, term_from_b.b, params)
    
    return RLWECiphertext(a=result_a, b=result_b)
...
def cmux(
    ...
    controlled = external_product(control, difference, params)
    return rlwe_add(controlled, false_ciphertext, params)
...
def blind_rotate(
    ...
    scaled_test_poly = scale_plaintext_poly(test_polynomial, params)
    
    rotated_by_minus_b = rlwe_monomial_mul(
        trivial_rlwe(scaled_test_poly, params),
        -rescaled.b,
        params,
    )
    
    accumulator = rotated_by_minus_b
    for i in range(params.k):
        a_hat_i = rescaled.a[i]
        encrypted_s_i = bootstrapping_key.encrypted_lwe_key_bits[i]
        
        false_branch = accumulator
        true_branch = rlwe_monomial_mul(accumulator, a_hat_i, params)
        
        accumulator = cmux(encrypted_s_i, false_branch, true_branch, params)
...
def key_switch(
    ...
    result_a: list[int] = [0] * params.k
    result_b = ciphertext.b
    
    for i in range(len(ciphertext.a)):
        coefficient = ciphertext.a[i]
        digits = gadget_decompose(coefficient, params)
        
        for j in range(params.l):
            digit = digits[j]
            ksk_entry = key_switching_key.encrypted_extracted_key[i][j]
            
            scaled_entry = lwe_scalar_mul(ksk_entry, digit, params)
            
            result_a = [normalize(result_a[k] - scaled_entry.a[k], params.q) for k in range(params.k)]
            result_b = normalize(result_b - scaled_entry.b, params.q)
    
    return LWECiphertext(a=result_a, b=result_b)
...
def programmable_bootstrap(
    ...
    switched = key_switch(extracted, evaluation_key.key_switching_key, params)
    return switched
...
def hom_nand(
    ...
    constant_one = trivial_lwe_plaintext(
        params.hom_nand_constant,
        params,
    )
    linear_preprocessing = lwe_sub(
        constant_one,
        lwe_add(left, right, params),
        params,
    )
