...
def decrypt_lwe(
    ...
    scaled_with_noise = normalize(ciphertext.b - inner, params.q)
    rounded = (scaled_with_noise + params.delta // 2) // params.delta
    return normalize(rounded, params.p)
...
def lwe_add(
    ...
    return LWECiphertext(a=new_a, b=normalize(left.b + right.b, params.q))
...
def lwe_scalar_mul(
    ...
    return LWECiphertext(a=new_a, b=normalize(scalar * ciphertext.b, params.q))
...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    for weight in gadget_weights(params):
        ...
        digits.append(digit)
        remainder = remainder - digit * weight
    return digits
...
def external_product(
    ...
    for j in range(params.l):
        term_a = rlwe_plain_mul(control.rows_for_a[j], decomposed_a[j], params)
        term_b = rlwe_plain_mul(control.rows_for_b[j], decomposed_b[j], params)
...
def blind_rotate(
    ...
    for index in range(params.k):
        ...
        control = bootstrapping_key.encrypted_lwe_key_bits[index]
...
def sample_extract(
    ...
    # に対応する LWE 暗号文を作る。
    new_a: list[int] = [normalize(ciphertext.a[0], params.q)]
    for index in range(1, params.n):
        new_a.append(normalize(-ciphertext.a[params.n - index], params.q))
    return LWECiphertext(a=new_a, b=normalize(ciphertext.b[0], params.q))
...
def key_switch(
    ...
    for index in range(len(ciphertext.a)):
        ...
        for j in range(params.l):
            term = lwe_scalar_mul(rows[j], digits[j], params)
...
def programmable_bootstrap(
    ...
    rotated = blind_rotate(
        ciphertext, test_polynomial, evaluation_key.bootstrapping_key, params
    )
