...
def encrypt_lwe(
    ...
    a = [rng.randrange(params.q) for _ in range(params.k)]
    b = dot_mod(a, secret_key, params.q)
    e = rng.randrange(params.noise_bound + 1)

    return LWECiphertext(a = a, b = normalize(b + scaled_message + e, params.q))
...
def decrypt_lwe(
    ...
    # TODO:
    # LWE暗号の復号を実装する
    scaled_m_with_noise = normalize(ciphertext.b - dot_mod(ciphertext.a, secret_key, params.q), params.q)
    rounded_plaintext = (scaled_m_with_noise + params.delta // 2) // params.delta
...
def lwe_add(
    ...
    # LWE 暗号文同士の加算を実装する
    return LWECiphertext(
        a = [normalize(l + r, params.q) for l, r in zip(left.a, right.a)],
        b = normalize(left.b + right.b, params.q)
    )
...
def lwe_scalar_mul(
    ...
    # LWE 暗号文のスカラー倍を実装する
    return LWECiphertext(
        a = [normalize(scalar * a, params.q) for a in ciphertext.a],
        b = normalize(scalar * ciphertext.b, params.q)
    )
...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    for w in gadget_weights(params):
        q = v // w
        g.append(q)
        v -= q * w
...
def external_product(
    ...
    for i in range(params.l):
       result = rlwe_add(result, rlwe_plain_mul(control.rows_for_a[i], decomposed_a[i], params), params)
       result = rlwe_add(result, rlwe_plain_mul(control.rows_for_b[i], decomposed_b[i], params), params)
...
def cmux(
    ...
    # に対応する暗号文演算を実装する。
    diff = rlwe_sub(true_ciphertext, false_ciphertext, params)
    scaled_diff = external_product(control, diff, params)

    return rlwe_add(scaled_diff, false_ciphertext, params)
...
def blind_rotate(
    ...
    rotation_b = monomial_poly(-rescaled.b, params)
    rotated_test_polynomial = poly_mul(rotation_b, scale_plaintext_poly(test_polynomial, params), params)
    acc = trivial_rlwe(rotated_test_polynomial, params)

    for i in range(params.k):
        rotated_a_poly = rlwe_monomial_mul(acc, rescaled.a[i], params)
        control = bootstrapping_key.encrypted_lwe_key_bits[i]
        acc = cmux(control, acc, rotated_a_poly, params)
...
def sample_extract(
    ...
    # に対応する LWE 暗号文を作る。
    a = []
    a.append(ciphertext.a[0])
    for i in range(params.n - 1, 0, -1):
        a.append(normalize(-1 * ciphertext.a[i], params.q))
...
def key_switch(
    ...
    for i in range(len(ciphertext.a)):
        decomposed_a = gadget_decompose(ciphertext.a[i], params)
        
        for j in range(params.l):
            term = lwe_scalar_mul(
                key_switching_key.encrypted_extracted_key[i][j],
                decomposed_a[j],
                params,
            )
    ...
    return key_switch(
        sample_extract(
            blind_rotate(
                ...
                params),
            params),
        evaluation_key.key_switching_key,
        params
    )
...
def hom_nand(
    ...
    constract_ct = trivial_lwe(scale_plaintext(params.hom_nand_constant, params), params)
    combined = lwe_sub(constract_ct, left, params)
    combined = lwe_sub(combined, right, params)

    return programmable_bootstrap(combined, evaluation_key, params)
