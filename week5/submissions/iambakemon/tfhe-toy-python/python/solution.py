...
def external_product(
    ...
    for j in range(params.l):
        # a(X) 側の行は復号式 b-as の -as に対応するため、
        # 位相へ -m*w_j*s(X) を、b(X) 側の行は +m*w_j を寄与する。
        # 両者を足すと m*(b(X)-a(X)s(X)) = m*phase になる。
        term_for_a = rlwe_plain_mul(
            control.rows_for_a[j],
            decomposed_a[j],
            params,
        )
        term_for_b = rlwe_plain_mul(
            control.rows_for_b[j],
            decomposed_b[j],
            params,
        )
        result = rlwe_add(result, term_for_a, params)
        result = rlwe_add(result, term_for_b, params)
    return result
...
def blind_rotate(
    ...
    # CMUX の制御 bit は bootstrapping key の RGSW_s'(s_i)。
    encrypted_key_bits = bootstrapping_key.encrypted_lwe_key_bits
    for index in range(len(encrypted_key_bits)):
        rotated = rlwe_monomial_mul(accumulator, rescaled.a[index], params)
        accumulator = cmux(
            encrypted_key_bits[index],
            accumulator,
            rotated,
            params,
        )
...
def sample_extract(
    ...
    return LWECiphertext(a=extracted_a, b=normalize(ciphertext.b[0], params.q))
...
def key_switch(
    ...
    # (0, b'') の位相は b'' なので、そこから <a'', s''> を引けば
    # 元の平文が残る。s''_i * q/B^{j+1} の暗号文が Key Switching Key。
    result = trivial_lwe(ciphertext.b, params)
    key_rows = key_switching_key.encrypted_extracted_key
    for index in range(len(ciphertext.a)):
        ...
        for j in range(len(digits)):
            ...
            if digit == 0:
                continue
            term = lwe_scalar_mul(key_rows[index][j], digit, params)
...
def hom_nand(
    ...
    # bit 0 -> p-1 = -1 のエンコードなので、両方 1 のときだけ
    # 1-1-1 = -1 = p-1 となり、それ以外は正の側に残る。
    constant = trivial_lwe_plaintext(params.hom_nand_constant, params)
    combined = lwe_sub(constant, left, params)
