...
def encrypt_lwe(
    ...
    b = 0
    for s in secret_key:
        a = rng.randrange(params.q)
        a_vec.append(a)
        b += a * s

    b += scaled_message
    b += rng.randrange(params.noise_bound + 1)
    b %= params.q

    return LWECiphertext(a=a_vec, b=b)
...
def decrypt_lwe(
    ...
    # TODO:
    # LWE暗号の復号を実装する
    a_s = dot_mod(ciphertext.a, secret_key, params.q)
    v = (ciphertext.b - a_s + params.delta // 2) // params.delta
    return v % params.p
...
def lwe_add(
    ...
    # LWE 暗号文同士の加算を実装する
    a = []
    for al, ar in zip(left.a, right.a):
        a.append((al + ar) % params.q)
    b = (left.b + right.b) % params.q
...
def lwe_scalar_mul(
    ...
    # TODO:
    # LWE 暗号文のスカラー倍を実装する
    a = [(v * scalar) % params.q for v in ciphertext.a]
    b = (ciphertext.b * scalar) % params.q
    return LWECiphertext(a=a, b=b)
...
def external_product(
    ...
    a_gadget = gadget_decompose_poly(ciphertext.a, params) # list[list[int]]
    b_gadget = gadget_decompose_poly(ciphertext.b, params)

    sum = []
    for i in range(len(a_gadget)):
        ct = rlwe_plain_mul(control.rows_for_a[i], a_gadget[i], params)
        sum.append(ct)

    for i in range(len(b_gadget)):
        ct = rlwe_plain_mul(control.rows_for_b[i], b_gadget[i], params)
        sum.append(ct)

    zero = RLWECiphertext(a=zero_poly(params), b=zero_poly(params))
    return reduce(lambda acc, v: rlwe_add(acc, v, params), sum, zero)
...
def cmux(
    ...
    v1 = rlwe_sub(true_ciphertext, false_ciphertext, params)
    v2 = external_product(control, v1, params)
    return rlwe_add(v2, false_ciphertext, params)
...
def blind_rotate(
    ...
    ciphertext = LWECiphertext(
        a = [v % (2 * params.n) for v in ciphertext.a],
        b = ciphertext.b % (2 * params.n),
    )

    shifted_tp = rlwe_monomial_mul(
        trivial_rlwe_plaintext(test_polynomial, params),
        -ciphertext.b,
        params
    )

    acc = shifted_tp
    for a, b in zip(ciphertext.a, bootstrapping_key.encrypted_lwe_key_bits):
        t = rlwe_monomial_mul(acc, a, params)
        f = acc
        acc = cmux(b, f, t, params)
...
def sample_extract(
    ...
    for i in range(params.n-1):
        a.append(-ciphertext.a[params.n-i-1] % params.q)
...
def key_switch(
    ...
    result = LWECiphertext(a=[0] * params.k, b=ciphertext.b)

    for i in range(len(ciphertext.a)):
        a_gadget = gadget_decompose(ciphertext.a[i], params)
        for j in range(len(a_gadget)):
            term = lwe_scalar_mul(
...
def hom_nand(
    ...
    one = trivial_lwe_plaintext(1, params)
    v1 = lwe_sub(one, left, params)
    v2 = lwe_sub(v1, right, params)
    return programmable_bootstrap(v2, evaluation_key, params)
