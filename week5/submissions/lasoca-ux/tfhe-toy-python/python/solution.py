...
def encrypt_lwe(
    ...
    # a は暗号文ごとに一様ランダムに選ぶ。
    a_vector: list[int] = []
    for _ in range(params.k): # params.k: LWE 秘密鍵 s=(s_0,...,s_{k-1}) の長さ
        a_vector.append(rng.randrange(params.q)) #params.p: LWE/RLWE の平文空間 Z_p の法

    # b=<a,s>+Delta*m+e (mod q) とする。
    # この toy 実装では、ノイズ e は 0 以上 noise_bound 以下の一様ランダム整数とする。
...
def lwe_add(
    ...
    for index in range(len(left.a)):
        new_a.append(normalize(left.a[index] + right.a[index], params.q))
...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    # value を Z_q の代表元に直し、B 進数の上位桁から取り出す。
...
def external_product(
    ...
    for j in range(params.l):
        # a 側と b 側の各 digit 多項式を、対応する RGSW の行に掛ける。
        product_for_a = rlwe_plain_mul(
            control.rows_for_a[j], decomposed_a[j], params
        )
        product_for_b = rlwe_plain_mul(
            control.rows_for_b[j], decomposed_b[j], params
        )
        result = rlwe_add(result, product_for_a, params)
        result = rlwe_add(result, product_for_b, params)
    return result
...
def cmux(
    ...
    # に対応する暗号文演算を実装する。
    # c_b * (c_1-c_0) + c_0 により、bit が 0 なら c_0、1 なら c_1 になる。
    difference = rlwe_sub(true_ciphertext, false_ciphertext, params)
    selected_difference = external_product(control, difference, params)
...
def blind_rotate(
    ...
    # Acc は最初 X^{-b_hat}v(X) を暗号化した自明な暗号文とする。
    scaled_test_polynomial = scale_plaintext_poly(test_polynomial, params)
    initial_poly = poly_mul(
        monomial_poly(-rescaled.b, params),
        scaled_test_polynomial,
        params,
    )
    accumulator = trivial_rlwe(initial_poly, params)

    for index in range(len(rescaled.a)):
        # s_i=0 なら現在値を維持し、s_i=1 なら X^{a_hat_i} 倍を選ぶ。
        rotated = rlwe_monomial_mul(accumulator, rescaled.a[index], params)
        accumulator = cmux(
...
def sample_extract(
    ...
    # に対応する LWE 暗号文を作る。
    # RLWE の定数項に寄与する係数を並べ替え、符号を反転する。
    extracted_a: list[int] = [normalize(ciphertext.a[0], params.q)]
    for index in range(params.n - 1, 0, -1):
        extracted_a.append(normalize(-ciphertext.a[index], params.q))
    return LWECiphertext(
        a=extracted_a,
...
def key_switch(
    ...
    # b'' を保った自明な暗号文から、a''_i s''_i の暗号化を順に引く。
    ...
    for coefficient_index in range(len(ciphertext.a)):
        ...
        for j in range(params.l):
            ...
            scaled_key_ciphertext = lwe_scalar_mul(
                key_ciphertext, digits[j], params
            )
            result = lwe_sub(result, scaled_key_ciphertext, params)
    return result
...
def programmable_bootstrap(
    ...
    # Blind Rotation で関数値を定数項へ移し、LWE 抽出後に元の鍵へ戻す。
...
def hom_nand(
    ...
    # 自明な暗号文 Enc(1) から左右の暗号文を引き、1-m_1-m_2 を作る。
    constant = trivial_lwe_plaintext(params.hom_nand_constant, params)
    preprocessed = lwe_sub(constant, left, params)
    preprocessed = lwe_sub(preprocessed, right, params)

    # PBS によって NAND の出力を bit の表現（p-1 または 1）へ戻す。
