...
def encrypt_lwe(
    ...
    b_value = normalize(
        dot_mod(a_vector, secret_key, params.q) + scaled_message + error,
        params.q,
    )
...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    """
    # value を q/B, q/B^2, ..., q/B^l の各桁に分解する。
...
def external_product(
    ...
    for j in range(params.l):
        ...
        a_product = rlwe_plain_mul(
            control.rows_for_a[j],
            decomposed_a[j],
            params,
        )
        b_product = rlwe_plain_mul(
        ...
        result = rlwe_add(result, a_product, params)
        result = rlwe_add(result, b_product, params)
    return result
...
def cmux(
    ...
    # CMUX(c_b,c_0,c_1)=c_b external_product (c_1-c_0)+c_0
...
def key_switch(
    ...
    for coefficient_index in range(len(ciphertext.a)):
        ...
        for j in range(params.l):
            encrypted_digit = lwe_scalar_mul(
                key_switching_key.encrypted_extracted_key[coefficient_index][j],
                digits[j],
                params,
            )
            result = lwe_sub(result, encrypted_digit, params)
    return result
