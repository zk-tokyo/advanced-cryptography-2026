...
def encrypt_lwe(
    ...
    b = scaled_message + sum(s_i * a_i for s_i, a_i in zip(secret_key, a)) + e
    return LWECiphertext(a=a, b=normalize(b, params.q))
...
def decrypt_lwe(
    ...
    # TODO:
    # LWE暗号の復号を実装する
    a_s = sum(s_i * a_i for s_i, a_i in zip(secret_key, ciphertext.a))
    message_with_noise = normalize(ciphertext.b - a_s, params.q)
    return normalize((message_with_noise + params.delta // 2) // params.delta, params.p)
...
def lwe_scalar_mul(
    ...
    # LWE 暗号文のスカラー倍を実装する
    return LWECiphertext(
        a=[normalize(scalar * a_i, params.q) for a_i in ciphertext.a],
...
def external_product(
    ...
    a_decomposed = gadget_decompose_poly(ciphertext.a, params)
    b_decomposed = gadget_decompose_poly(ciphertext.b, params)
    result = trivial_rlwe(zero_poly(params), params)
    for j in range(params.l):
        result = rlwe_add(
            result,
            rlwe_add(
                rlwe_plain_mul(control.rows_for_a[j], a_decomposed[j], params),
                rlwe_plain_mul(control.rows_for_b[j], b_decomposed[j], params),
                params,
            ),
            params,
        )
...
def cmux(
    ...
    # に対応する暗号文演算を実装する。
    return rlwe_add(
        external_product(control, rlwe_sub(true_ciphertext, false_ciphertext, params), params),
        false_ciphertext,
        params,
    )
...
def blind_rotate(
    ...
    c_scaled = rescale_lwe_ciphertext(ciphertext, params)
    q = trivial_rlwe_plaintext(test_polynomial, params)
    for j in range(params.k):
        a_hat_j = c_scaled.a[j]
        s_i = bootstrapping_key.encrypted_lwe_key_bits[j]
        q = cmux(s_i, q, rlwe_monomial_mul(q, a_hat_j, params), params)
    return rlwe_monomial_mul(q, -c_scaled.b, params)
...
def sample_extract(
    ...
    # に対応する LWE 暗号文を作る。
    a_extracted = [ciphertext.a[0]] + [normalize(-x, params.q) for x in reversed(ciphertext.a[1:])]
    b_extracted = ciphertext.b[0]
    return LWECiphertext(a=a_extracted, b=b_extracted)
...
def key_switch(
    ...
    for i in range(len(ciphertext.a)):
        ...
        for j in range(params.l):
            result = lwe_sub(result, lwe_scalar_mul(key_switching_key.encrypted_extracted_key[i][j], digits[j], params), params)
    return result
...
def hom_nand(
    ...
    r = lwe_sub(
            trivial_lwe_plaintext(params.hom_nand_constant, params),
            lwe_add(left, right, params),
            params,
        )
    return programmable_bootstrap(r, evaluation_key, params)
