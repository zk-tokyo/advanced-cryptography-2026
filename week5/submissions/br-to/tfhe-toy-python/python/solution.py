...
def sample_extract(
    ...
    for index in range(1, params.n):
        coefficient = normalize(-ciphertext.a[params.n - index], params.q)
        extracted_a.append(coefficient)
    ...
    for index in range(len(ciphertext.a)):
        ...
        for digit_index in range(len(digits)):
            ...
            if digit == 0:
                continue
            term = lwe_scalar_mul(
                key_switching_key.encrypted_extracted_key[index][digit_index],
                digit,
                params,
            )
...
def programmable_bootstrap(
    ...
    test_polynomial: list[int] = list(params.nand_test_polynomial)
...
def hom_nand(
    ...
    summed = lwe_add(left, right, params)
    constant_ciphertext = trivial_lwe_plaintext(params.hom_nand_constant, params)
    linear = lwe_sub(constant_ciphertext, summed, params)
