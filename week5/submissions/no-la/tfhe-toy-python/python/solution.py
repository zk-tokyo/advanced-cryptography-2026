...
def external_product(
    ...
    for j in range(params.l):
        a_part = rlwe_plain_mul(control.rows_for_a[j], decomposed_a[j], params)
        b_part = rlwe_plain_mul(control.rows_for_b[j], decomposed_b[j], params)
        result = rlwe_add(result, a_part, params)
        result = rlwe_add(result, b_part, params)
    return result
...
def cmux(
    ...
    return rlwe_add(selected_difference, false_ciphertext, params)
...
def blind_rotate(
    ...
    for index in range(len(rescaled.a)):
        exponent = rescaled.a[index]
        rotated = rlwe_monomial_mul(accumulator, exponent, params)
...
def key_switch(
    ...
    for coefficient_index in range(len(ciphertext.a)):
        ...
        for j in range(len(digits)):
            weighted_key = lwe_scalar_mul(
                key_switching_key.encrypted_extracted_key[coefficient_index][j],
                digits[j],
                params,
            )
            result = lwe_sub(result, weighted_key, params)
    return result
