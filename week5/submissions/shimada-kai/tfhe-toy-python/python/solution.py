...
def decrypt_lwe(
    ...
    scaled_message_with_noise = normalize(ciphertext.b - dot_mod(ciphertext.a, secret_key, params.q), params.q)
...
def lwe_scalar_mul(
    ...
    new_a = []
    for value in ciphertext.a:
        new_value = normalize(scalar*value, params.q)
...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    for weight in gadget_weights(params):
        ...
        digits.append(digit)
        remaining = remaining - digit * weight
    return digits
...
def external_product(
    ...
    for j in range(params.l):
        term_a = rlwe_plain_mul(
            control.rows_for_a[j],
            decomposed_a[j],
            params
        )

        term_b = rlwe_plain_mul(
...
def cmux(
    ...
    mul = external_product(control, difference, params)
    result = rlwe_add(false_ciphertext, mul, params)
    return result
...
def blind_rotate(
    ...
    scaled_test_polynomial = scale_plaintext_poly(
        test_polynomial,
        params,
    )

    accumulator = trivial_rlwe(
        scaled_test_polynomial,
        params,
    )

    accumulator = rlwe_monomial_mul(
        accumulator,
        -b_hat,
        params,
    )
...
def hom_nand(
    ...
    constant = trivial_lwe_plaintext(
