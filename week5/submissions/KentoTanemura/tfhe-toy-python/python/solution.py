...
def cmux(
    ...
    controlled_difference = external_product(control, difference, params)
    return rlwe_add(false_ciphertext, controlled_difference, params)
...
def blind_rotate(
    ...
    for index, rotation in enumerate(rescaled.a):
        rotated = rlwe_monomial_mul(accumulator, rotation, params)
        accumulator = cmux(
...
def key_switch(
    ...
    for coefficient_index, coefficient in enumerate(ciphertext.a):
        digits = gadget_decompose(coefficient, params)
        for j, digit in enumerate(digits):
            if digit == 0:
                continue
            key_term = lwe_scalar_mul(
                key_switching_key.encrypted_extracted_key[coefficient_index][j],
                digit,
                params,
            )
            result = lwe_sub(result, key_term, params)
    return result
...
def hom_nand(
    ...
    linear_result = lwe_sub(constant, left, params)
    linear_result = lwe_sub(linear_result, right, params)
    return programmable_bootstrap(linear_result, evaluation_key, params)
