...
def encrypt_lwe(
    ...
    b = normalize(dot_mod(a_vector, secret_key, params.q) + scaled_message + error, params.q)
    return LWECiphertext(a=a_vector, b=b)
...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    """
    remainder = normalize(value, params.q)
    digits: list[int] = []
...
def external_product(
    ...
    for j in range(params.l):
        term_a = rlwe_plain_mul(control.rows_for_a[j], a_digits[j], params)
        result = rlwe_add(result, term_a, params)
        term_b = rlwe_plain_mul(control.rows_for_b[j], b_digits[j], params)
        result = rlwe_add(result, term_b, params)
    return result
...
def cmux(
    ...
    product = external_product(control, diff, params)
    return rlwe_add(product, false_ciphertext, params)
...
def blind_rotate(
    ...
    scaled_test_polynomial = scale_plaintext_poly(list(test_polynomial), params)
    initial_poly = poly_mul(
        scaled_test_polynomial,
        monomial_poly(-rescaled.b, params),
        params,
    )
    acc = trivial_rlwe(initial_poly, params)

    for index in range(params.k):
        rotated = rlwe_monomial_mul(acc, rescaled.a[index], params)
        acc = cmux(
...
def sample_extract(
    ...
    a_extracted: list[int] = [ciphertext.a[0]]
    for index in range(1, params.n):
        value = normalize(-ciphertext.a[params.n - index], params.q)
        a_extracted.append(value)
    return LWECiphertext(a=a_extracted, b=ciphertext.b[0])
...
def key_switch(
    ...
    for _ in range(params.k):
        zero_a.append(0)
    result = LWECiphertext(a=zero_a, b=normalize(ciphertext.b, params.q))

    for index in range(len(ciphertext.a)):
        digits = gadget_decompose(ciphertext.a[index], params)
        rows = key_switching_key.encrypted_extracted_key[index]
        for digit, row in zip(digits, rows):
            if digit == 0:
                continue
            result = lwe_sub(result, lwe_scalar_mul(row, digit, params), params)
    return result
...
def hom_nand(
    ...
    constant_ciphertext = trivial_lwe_plaintext(params.hom_nand_constant, params)
    combined = lwe_sub(
        lwe_sub(constant_ciphertext, left, params),
        right,
        params,
    )
