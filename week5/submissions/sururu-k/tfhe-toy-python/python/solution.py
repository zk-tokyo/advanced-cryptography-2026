...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    v = normalize(value, params.q)
    digits: list[int] = []
    for weight in gadget_weights(params):
        digits.append(v // weight)
        v %= weight
    return digits
...
def cmux(
    ...
    prod = external_product(control, diff, params)
    return rlwe_add(false_ciphertext, prod, params)
...
def blind_rotate(
    ...
    acc = trivial_rlwe(scaled_test_poly, params)
    acc = rlwe_monomial_mul(acc, -rescaled.b, params)
    for i in range(params.k):
        false_ct = acc
        true_ct = rlwe_monomial_mul(acc, rescaled.a[i], params)
        acc = cmux(bootstrapping_key.encrypted_lwe_key_bits[i], false_ct, true_ct, params)
    return acc
...
def key_switch(
    ...
    for i in range(params.n):
        ...
        for j in range(params.l):
            term = lwe_scalar_mul(key_switching_key.encrypted_extracted_key[i][j], digits[j], params)
...
def hom_nand(
    ...
    c_const = trivial_lwe_plaintext(params.hom_nand_constant, params)
    c_linear = lwe_sub(lwe_sub(c_const, left, params), right, params)
    return programmable_bootstrap(c_linear, evaluation_key, params)
