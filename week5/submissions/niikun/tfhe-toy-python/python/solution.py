...
def encrypt_lwe(
    ...
    inner = dot_mod(a, secret_key,params.q)
    e = rng.randrange(params.noise_bound + 1)
    b = normalize(inner + scaled_message + e, params.q)
...
def decrypt_lwe(
    ...
    a = ciphertext.a
    b = ciphertext.b
    left = normalize(b - dot_mod(a, secret_key,params.q),params.q)
    m = normalize((left + params.delta//2)//params.delta, params.p)
    return m
...
def lwe_scalar_mul(
    ...
    # TODO:
    # LWE 暗号文のスカラー倍を実装する
    a = [normalize(a_ * scalar, params.q) for a_ in ciphertext.a]
    b = normalize(ciphertext.b * scalar,params.q)
    return LWECiphertext(a=a,b=b)
...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    value = normalize(value, params.q) 
    result = [0 for _ in range(params.l)]
    remaining_value = value
    for i in range(params.l):
        divisor = params.q // (params.B ** (i+1))
        res = remaining_value // divisor
...
def external_product(
    ...
    digits_a = gadget_decompose_poly(ciphertext.a, params)
    digits_b = gadget_decompose_poly(ciphertext.b, params)

    result = RLWECiphertext(a=zero_poly(params), b=zero_poly(params))
    for j in range(params.l):
        result = rlwe_add(
            result,
            rlwe_plain_mul(control.rows_for_a[j], digits_a[j], params),
            params,
        )
        result = rlwe_add(
            result,
            rlwe_plain_mul(control.rows_for_b[j], digits_b[j], params),
            params,
        )
    return result
...
def blind_rotate(
    ...
    scaled_test = scale_plaintext_poly(test_polynomial, params)
    accumulator = trivial_rlwe(scaled_test, params)
    accumulator = rlwe_monomial_mul(accumulator, -rescaled.b, params)

    for i in range(len(rescaled.a)):
        rotated = rlwe_monomial_mul(accumulator, rescaled.a[i], params)
        control = bootstrapping_key.encrypted_lwe_key_bits[i]
        accumulator = cmux(control, accumulator, rotated, params)
...
def sample_extract(
    ...
    for k in range(1, n):
        extracted_a.append(normalize(-ciphertext.a[n - k], params.q))
    return LWECiphertext(
        a=extracted_a,
...
def key_switch(
    ...
    result = trivial_lwe(ciphertext.b, params)   # (0, b'')
    ksk = key_switching_key.encrypted_extracted_key
    for i in range(len(ciphertext.a)):
        digits = gadget_decompose(ciphertext.a[i], params)
        for j in range(params.l):
            term = lwe_scalar_mul(ksk[i][j], digits[j], params)
...
def hom_nand(
    ...
    preprocessed = trivial_lwe_plaintext(params.hom_nand_constant, params)  # (0, Δ·1)
