...
def encrypt_lwe(
    ...
    e = rng.randint(0, params.noise_bound)
...
def decrypt_lwe(
    ...
    # TODO:
    # LWE暗号の復号を実装する
    phase = normalize(ciphertext.b - dot_mod(ciphertext.a, secret_key, params.q),
  params.q)
    return round(phase / params.delta) % params.p
...
def lwe_add(
    ...
    # TODO:
    # LWE 暗号文同士の加算を実装する
    a = [normalize(x + y, params.q) for x, y in zip(left.a, right.a)]
    return LWECiphertext(a=a, b=normalize(left.b + right.b, params.q))
...
def lwe_scalar_mul(
    ...
    # TODO:
    # LWE 暗号文のスカラー倍を実装する
    a = [normalize(x * scalar, params.q) for x in ciphertext.a]
    return LWECiphertext(a=a, b=normalize(ciphertext.b * scalar, params.q))
...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    for weight in gadget_weights(params):
        digits.append(remaining // weight)
        remaining %= weight
    return digits
...
def external_product(
    ...
    for digit_poly, row in zip(decomposed_a, control.rows_for_a):
        result = rlwe_add(result, rlwe_plain_mul(row, digit_poly, params), params)
    for digit_poly, row in zip(decomposed_b, control.rows_for_b):
        result = rlwe_add(result, rlwe_plain_mul(row, digit_poly, params), params)
    return result
...
def cmux(
    ...
    return rlwe_add(external_product(control, difference, params),
  false_ciphertext, params)
...
def blind_rotate(
    ...
    accumulator = trivial_rlwe_plaintext(test_polynomial, params)
    accumulator = rlwe_monomial_mul(accumulator, -rescaled.b, params)

    for index, encrypted_bit in enumerate(bootstrapping_key.encrypted_lwe_key_bits):
        rotated = rlwe_monomial_mul(accumulator, rescaled.a[index], params)
        accumulator = cmux(encrypted_bit, accumulator, rotated, params)
    return accumulator
...
def sample_extract(
    ...
    # に対応する LWE 暗号文を作る。
    a = [ciphertext.a[0]] + [
        normalize(-ciphertext.a[params.n - index], params.q)
        for index in range(1, params.n)
    ]
    return LWECiphertext(a=a, b=normalize(ciphertext.b[0], params.q))
...
def key_switch(
    ...
    for index, value in enumerate(ciphertext.a):
        digits = gadget_decompose(value, params)
        for level, digit in enumerate(digits):
            if digit == 0:
                continue
            entry = key_switching_key.encrypted_extracted_key[index][level]
            result = lwe_sub(result, lwe_scalar_mul(entry, digit, params), params)
    return result
...
def hom_nand(
    ...
    linear = lwe_sub(lwe_sub(constant, left, params), right, params)
