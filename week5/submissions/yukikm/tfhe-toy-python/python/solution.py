...
def encrypt_lwe(
    ...
    # TODO:
    # LWE暗号の暗号化を実装する
    # a は Z_q^k から一様に選ぶ。秘密鍵は 0/1 ベクトルなので、
    # dot_mod で通常の内積を計算してから q で正規化すればよい。
    a_vector: list[int] = []
    for _ in range(params.k):
        a_vector.append(rng.randrange(params.q))

    # この toy 実装の誤差は非負の e in {0, ..., noise_bound} である。
    # LWE の規約を b=<a,s>+Delta*m+e としておくことで、復号時に
    # b-<a,s> を計算すると Delta*m+e だけが残る。
    error = rng.randrange(params.noise_bound + 1)
    inner_product = dot_mod(a_vector, secret_key, params.q)
    b_value = normalize(inner_product + scaled_message + error, params.q)
...
def decrypt_lwe(
    ...
    # TODO:
    # LWE暗号の復号を実装する
    # 暗号化で加えた <a,s> を引き、Z_q 上の Delta*m+e を得る。
    scaled_message_with_noise = normalize(
        ciphertext.b - dot_mod(ciphertext.a, secret_key, params.q),
        params.q,
    )

    # Delta の半分を足してから整数除算することで、最も近い Z_p の
    # 平文へ丸める。q 付近で p へ丸め上がる場合も最後の mod p で扱う。
    rounded_plaintext = (
        scaled_message_with_noise + params.delta // 2
    ) // params.delta
...
def lwe_add(
    ...
    # TODO:
    # LWE 暗号文同士の加算を実装する
    # LWE は線形なので a の各成分と b をそれぞれ加算すれば、
    # 復号される平文同士の和を暗号化した暗号文になる。
    new_a: list[int] = []
...
def lwe_scalar_mul(
    ...
    # TODO:
    # LWE 暗号文のスカラー倍を実装する
    # a と b の両方へ同じスカラーを掛けると、復号式
    # b-<a,s> 全体にもそのスカラーが掛かる。
    new_a: list[int] = []
    for value in ciphertext.a:
        new_a.append(normalize(value * scalar, params.q))
...
def gadget_decompose(value: int, params: ToyTFHEParams) -> list[int]:
    ...
    # q=B^l という課題の前提を gadget_weights 内でも検証する。
    # value はまず Z_q の標準代表元へ直し、上位桁から順に取り出す。
    remaining = normalize(value, params.q)
    digits: list[int] = []
    for weight in gadget_weights(params):
        digit = remaining // weight
        digits.append(digit)
...
def external_product(
    ...
    # RGSW の a 側の行は復号すると -m*g_j*s、b 側の行は
    # m*g_j になる。各 digit 多項式を掛けて全行を足すことで、
    # m * (b-a*s)、すなわち制御 bit と入力平文の積が得られる。
    result = trivial_rlwe(zero_poly(params), params)
    for j in range(params.l):
        product_from_a = rlwe_plain_mul(
            control.rows_for_a[j],
            decomposed_a[j],
            params,
        )
        product_from_b = rlwe_plain_mul(
            control.rows_for_b[j],
            decomposed_b[j],
            params,
        )
        result = rlwe_add(result, product_from_a, params)
        result = rlwe_add(result, product_from_b, params)
    return result
...
def cmux(
    ...
    # に対応する暗号文演算を実装する。
    # control=0 なら差分の項が消えて false 側が残り、control=1 なら
    # (true-false)+false となって true 側が残る。
    branch_difference = rlwe_sub(true_ciphertext, false_ciphertext, params)
    selected_difference = external_product(control, branch_difference, params)
    return rlwe_add(false_ciphertext, selected_difference, params)
...
def blind_rotate(
    ...
    # v(X) は Z_p の係数なので Delta 倍して Z_q に配置する。
    # まず既知の -b_hat 回転を行い、この時点の暗号文を accumulator とする。
    ...
    # s_i は BSK の中で RGSW 暗号化されている。CMUX により、s_i=0 なら
    # accumulator を維持し、s_i=1 なら X^{a_hat_i} 倍した方を選ぶ。
    # 全成分の処理後は X^(-b_hat+<a_hat,s>) v(X) になる。
    for index in range(len(rescaled.a)):
        rotated_if_secret_bit_is_one = rlwe_monomial_mul(
            accumulator,
            rescaled.a[index],
            params,
        )
        accumulator = cmux(
            bootstrapping_key.encrypted_lwe_key_bits[index],
            accumulator,
            rotated_if_secret_bit_is_one,
            params,
        )
    return accumulator
...
def sample_extract(
    ...
    # に対応する LWE 暗号文を作る。
    # negacyclic 積 a(X)s(X) の定数項は
    # a_0*s_0-a_{N-1}*s_1-...-a_1*s_{N-1} となる。
    # したがって、この係数列を通常の LWE の a ベクトルへ並べ直す。
    extracted_a = [normalize(ciphertext.a[0], params.q)]
    for index in range(1, params.n):
        coefficient = -ciphertext.a[params.n - index]
        extracted_a.append(normalize(coefficient, params.q))
    return LWECiphertext(
        a=extracted_a,
        b=normalize(ciphertext.b[0], params.q),
    )
...
def key_switch(
    ...
    # KSK[i][j] は旧鍵の s''_i*g_j を新しい鍵で暗号化している。
    # b'' だけを持つ自明な暗号文から a'' の各桁に対応する KSK を
    # 引けば、復号値 b''-<a'',s''> を保ったまま鍵だけを変更できる。
    switched = trivial_lwe(ciphertext.b, params)
    for coefficient_index in range(len(ciphertext.a)):
        digits = gadget_decompose(ciphertext.a[coefficient_index], params)
        for j in range(params.l):
            encrypted_term = lwe_scalar_mul(
                key_switching_key.encrypted_extracted_key[coefficient_index][j],
                digits[j],
                params,
            )
            switched = lwe_sub(switched, encrypted_term, params)
    return switched
...
def programmable_bootstrap(
    ...
    # NAND 用のテスト多項式で入力位相を Blind Rotation し、その定数項を
    # LWE として抽出した後、元の LWE 秘密鍵の暗号文へ Key Switching する。
    test_polynomial = list(params.nand_test_polynomial)
...
def hom_nand(
    ...
    # 定数 1 は a=0 の自明な暗号文として用意する。LWE の線形性により、
    # 暗号文同士の減算だけで Delta*(1-m_1-m_2) の位相を作れる。
    linear_term = trivial_lwe_plaintext(params.hom_nand_constant, params)
    linear_term = lwe_sub(linear_term, left, params)
    linear_term = lwe_sub(linear_term, right, params)

    # PBS が位相に応じてテスト多項式の +1 または -1 (=p-1) を選び、
    # 同時に出力を新しい LWE 暗号文へ更新する。
    return programmable_bootstrap(linear_term, evaluation_key, params)
