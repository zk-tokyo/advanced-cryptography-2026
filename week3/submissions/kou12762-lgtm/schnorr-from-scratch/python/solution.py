...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if x_P % curve.p == x_Q % curve.p and (y_P + y_Q) % curve.p == 0:
        return None
    
    if P == Q:
        lam = field_mul(field_add(3*x_P**2 ,curve.a,curve.p),field_inv(2*y_P,curve.p),curve.p)
    else:
        lam = field_mul(field_add(y_Q,-y_P,curve.p),field_inv(field_add(x_Q,-x_P,curve.p),curve.p),curve.p)
...
def ec_scalar_mul(k: int, P: Point, curve: Curve) -> Point:
    ...
    result = INFINITY   # 足し込み先。単位元から始める
    addend = P          # ループ i 回目で P の 2^i 倍
