...
def field_inv(a: int, p: int) -> int:
    ...
    """
    if (a % p == 0):
        raise ValueError("a must not be the multiple of p.")
...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if (P == Q):
        slope = (3 * P[0] * P[0] + curve.a) * field_inv(2 * P[1], curve.p)
    else:
        slope = (P[1] - Q[1]) * field_inv(P[0] - Q[0], curve.p)
    x = slope ** 2 - (P[0] + Q[0])
    y = slope * (x - P[0]) + P[1]
...
def ec_scalar_mul(k: int, P: Point, curve: Curve) -> Point:
    ...
    while (k != 0):
        if (k & 1):
            ans = ec_add(ans, i_pow, curve)
        i_pow = ec_add(i_pow, i_pow, curve)
        k = k >> 1
...
def schnorr_verify(
    ...
    """
    e = challenge_hash(signature[0], pubkey, message, curve.n)
    return sigma_verify(pubkey, signature[0], e, signature[1], curve)
