...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    # x_P == x_Qかつy_P + y_Q == 0 (mod p)ならINFINITYを返す
    if (x_P == x_Q) and ((y_P + y_Q) % curve.p) == 0:
        return INFINITY
    
    if P == Q:
        # 接線の傾き
        lam = ((3*x_P**2 + curve.a) * field_inv(2*y_P, curve.p)) % curve.p
    else:
        # 弦の傾き
        lam = ((y_Q - y_P) * field_inv(x_Q - x_P, curve.p)) % curve.p

    # 傾きlamから(x_R, y_R)を求める
    x_R = lam**2 - x_P - x_Q
    y_R = lam * (x_P - x_R) - y_P
...
def sigma_verify(pubkey: Point, R: Point, e: int, s: int, curve: Curve) -> bool:
    ...
    """
    l = ec_scalar_mul(s, curve.G, curve)
    r = ec_add(
        R,
        ec_scalar_mul(e, pubkey, curve),
        curve
    )
    return l == r
