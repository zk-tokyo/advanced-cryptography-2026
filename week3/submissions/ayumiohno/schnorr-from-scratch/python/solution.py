...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if Q is INFINITY:
        return P
    if P[0] == Q[0] and (P[1] + Q[1]) % curve.p == 0:
        return INFINITY
    if P == Q:
        lam = field_mul(3 * P[0] ** 2 + curve.a, field_inv(2 * P[1], curve.p), curve.p)
    else:
        lam = field_mul(Q[1] - P[1], field_inv(Q[0] - P[0], curve.p), curve.p)
    x_R = (lam ** 2 - P[0] - Q[0]) % curve.p
    y_R = (lam * (P[0] - x_R) - P[1]) % curve.p
    R = (x_R, y_R)
    assert is_on_curve(R, curve), f"R={R} is not on curve"
    return R
    raise NotImplementedError
...
def ec_scalar_mul(k: int, P: Point, curve: Curve) -> Point:
    ...
    addend = P
    while k > 0:
        quatient, remainder = divmod(k, 2)
        res = ec_add(res, addend, curve) if remainder else res
