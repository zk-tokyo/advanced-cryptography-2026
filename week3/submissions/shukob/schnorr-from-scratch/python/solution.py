...
def ec_add_impl(slope: int, P: Point, Q: Point, curve: Curve) -> Point:
    if point_equals(P, INFINITY):
        return Q
    if point_equals(Q, INFINITY):
        return P

    x_P, y_P = P
    x_Q, y_Q = Q

    x_R = slope**2 - field_add(x_P, x_Q, curve.p)
    y_R = field_add(slope * field_add(x_P, -x_R, curve.p), - y_P, curve.p)

    return (x_R % curve.p, y_R % curve.p)
...
def field_inv(a: int, p: int) -> int:
    ...
    g, x, y = extended_gcd(a % p, p)
    if g != 1:
        raise ValueError("逆元が存在しません")
    return x % p

def chord_slope(P: Point, Q: Point, p: int) -> int:
    return 0 if P is None or Q is None else field_mul(field_add(Q[1],-P[1], p), field_inv(field_add(Q[0], -P[0], p), p), p)

def ec_negate(P: Point, curve: Curve) -> Point:
    """点の逆元 -P = (x, -y) です。無限遠点はそれ自身が逆元です。"""
    if P is None:
        return INFINITY
    x, y = P
    return (x, (-y) % curve.p)

def tangent_slope(P: Point, curve: Curve) -> int:
    if P is None:
        return 0
    x, y = P
    return field_mul(field_add(field_mul(3, field_mul(x, x, curve.p), curve.p), curve.a, curve.p), field_inv(field_mul(2, y, curve.p), curve.p), curve.p)
...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if point_equals(P, INFINITY):
        return Q 
    elif point_equals(Q, INFINITY):
        return P
    elif P[0] == Q[0] and (P[1] + Q[1]) % curve.p == 0:
        return INFINITY
    elif point_equals(P, Q):
        slope = tangent_slope(P, curve) % curve.p
    else:
        slope = chord_slope(P, Q, curve.p) % curve.p
    return ec_add_impl(slope, P, Q, curve)


def ec_scalar_mul(k: int, P: Point, curve: Curve) -> Point:
    """スカラー倍 k * P です(k は整数。負でも計算します)。
    ...
    k が負のときは (-k)*P = -(k*P) なので、絶対値で計算して逆元を返します。
    ...
    """
    if k < 0:  # (-k)*P = -(k*P) なので、絶対値で計算して最後に逆元を取る
        return ec_negate(ec_scalar_mul(-k, P, curve), curve)
    assert k >= 0
    result = INFINITY
    current = P 
    k_bin = bin(k)[2:]
    for i in reversed(range(len(k_bin))):  # 下位ビットから順に処理
        if k_bin[i] == '1':
            result = ec_add(result, current, curve)  # ビットが 1 の場合、結果に加算
        if i == 0:
            break
...
def sigma_verify(pubkey: Point, R: Point, e: int, s: int, curve: Curve) -> bool:
    ...
    """
    lhs = ec_scalar_mul(s, curve.G, curve)
    rhs = ec_add(R, ec_scalar_mul(e, pubkey, curve), curve)
    return point_equals(lhs, rhs)
