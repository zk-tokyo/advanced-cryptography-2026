...
def field_inv(a: int, p: int) -> int:
    ...
    a_mod = a % p
    if a_mod == 0:
        raise ValueError("0 does not have an inverse in F_p")
...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if P == Q:
        if y_P % curve.p == 0:
            return INFINITY
        slope = ((3 * x_P * x_P + curve.a) * field_inv((2 * y_P) % curve.p, curve.p)) % curve.p
    else:
        slope = ((y_Q - y_P) * field_inv((x_Q - x_P) % curve.p, curve.p)) % curve.p

    x_R = (slope * slope - x_P - x_Q) % curve.p
    y_R = (slope * (x_P - x_R) - y_P) % curve.p
    return (x_R, y_R)
