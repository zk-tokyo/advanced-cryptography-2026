...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if x_p == x_q and field_add(y_p, y_q, p) == 0:
        return INFINITY

    if P == Q:
        num = field_mul(3, field_mul(x_p, x_p, p), p)
        num = field_add(num, curve.a, p)
        den = field_inv(field_mul(2, y_p, p), p)
        lam = field_mul(num, den, p)
    else:
        num = (y_q - y_p) % p
        den = field_inv((x_q - x_p) % p, p)
        lam = field_mul(num, den, p)
