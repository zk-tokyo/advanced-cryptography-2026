...
def field_inv(a: int, p: int) -> int:
    ...
    あるので注意してください。
    """
    # ec_add が負の分母を渡すので、範囲外の a が来る前提で正規化する。
    residue = a % p
    if residue == 0:
        raise ValueError(f"{a} は F_{p} で 0 なので逆元は存在しません")

    g, x, _ = extended_gcd(residue, p)
    # p が素数なら到達しない。Part 3 が法 n で呼ぶ経路のための保険。
    if g != 1:
        raise ValueError(f"{a} と {p} は互いに素ではないので逆元は存在しません")
    return x % p
...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    # P == Q ではなく x で分岐する。正規化前の座標だとタプル比較が外れるため。
    # 曲線上で x_P == x_Q なら y_P == ±y_Q に限られ、下の 2 分岐で尽くせる。
    if (x_P - x_Q) % p == 0:
        # 互いに逆元。y == 0 の点の 2 倍算もここに落ちる。
        if (y_P + y_Q) % p == 0:
            return INFINITY
        # y_P == y_Q が確定するので 2 倍算。
...
def ec_scalar_mul(k: int, P: Point, curve: Curve) -> Point:
    ...
    """
    if k < 0:
        raise ValueError("k は 0 以上の整数にしてください")
    ...
    while k:
        ...
        if k:
            # 最上位ビット処理後の addend は未使用。正しさには影響しない。
...
def sigma_commit(r: int, curve: Curve) -> Point:
    ...
    """
    # ec_scalar_mul は k >= 0 前提。位数が n なので余りにしても R は不変。
...
def schnorr_sign(
    ...
    # ハッシュ入力の順序は schnorr_verify 側と揃えること。片方だけ変えると
    # 既存の署名がすべて検証不能になる。
    e = challenge_hash(R, pubkey, message, curve.n)
...
def schnorr_verify(
    ...
    """
    R, s = signature
    # 署名側の e を信用すると、別メッセージ用の s を流用できてしまう。
    e = challenge_hash(R, pubkey, message, curve.n)
