...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if P == Q:
        lamd = ((3*x_P**2 + curve.a) % curve.p) * field_inv(2*y_P, curve.p)
    else:
        lamd = ((y_Q - y_P) % curve.p) * field_inv(x_Q - x_P, curve.p)

    x_R = (lamd**2 - x_P - x_Q) % curve.p
    y_R = (lamd * (x_P - x_R) - y_P) % curve.p
...
def sigma_verify(pubkey: Point, R: Point, e: int, s: int, curve: Curve) -> bool:
    ...
    """
    return sigma_commit(s, curve) == ec_add(R, ec_scalar_mul(e, pubkey, curve), curve)
