...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    x_R = (lam**2 - x_P - x_Q) % curve.p
    y_R = (lam * (x_P - x_R) - y_P) % curve.p
...
def ec_scalar_mul(k: int, P: Point, curve: Curve) -> Point:
    ...
    while k > 0:
        if k % 2:
            sum = ec_add(sum, doubling, curve)

        k = k >> 1
        doubling = ec_add(doubling, doubling, curve)
...
def sigma_verify(pubkey: Point, R: Point, e: int, s: int, curve: Curve) -> bool:
    ...
    """
    p1 = ec_scalar_mul(s, curve.G, curve)
    p2 = ec_add(R, ec_scalar_mul(e, pubkey, curve), curve)
...
def schnorr_verify(
    ...
    p1 = ec_scalar_mul(signature[1], curve.G, curve)
    p2 = ec_add(signature[0], ec_scalar_mul(e, pubkey, curve), curve)
    return p1[0] == p2[0] and p1[1] == p2[1]
