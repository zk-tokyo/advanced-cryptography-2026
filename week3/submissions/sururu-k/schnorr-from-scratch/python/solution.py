...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if x_P == x_Q and y_P == y_Q:
        numer = field_add(field_mul(3, field_mul(x_P, x_P, p), p), curve.a, p)
        denom = field_mul(2, y_P, p)
    else:
        numer = field_add(y_Q, -y_P, p)
        denom = field_add(x_Q, -x_P, p)

    lam = field_mul(numer, field_inv(denom, p), p)
...
def sigma_response(x: int, r: int, e: int, curve: Curve) -> int:
    ...
    """
    n = curve.n
    return field_add(r, field_mul(e, x, n), n)
