...
def field_inv(a: int, p: int) -> int:
    ...
    gcd, inverse, _ = extended_gcd(a, p)
    if gcd != 1:
        raise ValueError(f"{a} has no multiplicative inverse modulo {p}")
    return inverse % p
...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    # A vertical line meets the curve at the point at infinity.  This also
    # covers doubling a point whose y-coordinate is zero.
    ...
    if P == Q:
        ...
        numerator = field_add(y_Q, -y_P, p)
        denominator = field_add(x_Q, -x_P, p)
