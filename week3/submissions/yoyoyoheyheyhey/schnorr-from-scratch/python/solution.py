...
def field_inv(a: int, p: int) -> int:
    ...
    あるので注意してください。
    """
    # 有限体では a と a % p は同じ要素として扱う。
    a_normalized = a % p

    # 0 については 0 * x == 1 (mod p) を満たす x が存在しない。
    if a_normalized == 0:
        raise ValueError("0 has no multiplicative inverse")

    # extended_gcd は次を満たす整数 x, y を求める。
    #     a_normalized * x + p * y == gcd(a_normalized, p)
    gcd, x, _y = extended_gcd(a_normalized, p)
    if gcd != 1:
        raise ValueError("a has no multiplicative inverse modulo p")

    # gcd == 1 なら、上の式を mod p で見ることで
    #     a_normalized * x == 1 (mod p)
    # となる。x は負の場合もあるので 0..p-1 に正規化する。
    return x % p
...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    # ここまで来れば P と Q はどちらも (x, y) のタプル。
    x_P, y_P = P
    x_Q, y_Q = Q
    p = curve.p

    # 2. x が同じで y が互いに逆符号 → 無限遠点（P + (−P) = O）
    # 逆符合: y_P + y_Q == 0 (例: 298 + (-298) == 0)
    # modの世界なので、pで割った余りが0なら逆符号として判定する
    if x_P == x_Q and (y_P + y_Q) % p == 0:
        return INFINITY

    if P == Q:
        # 3. P == Q → 2 倍算（接線の傾き λ = (3x² + a) / 2y）
        # P == Q なら P + Q = P + P = 2P だから2倍算
        numerator = 3 * (x_P ** 2) + curve.a
        denominator = 2 * y_P
    else:
...
def ec_scalar_mul(k: int, P: Point, curve: Curve) -> Point:
    ...
    # k を 2 進数で見て下位ビットから順に ...
    bits = [int(bit) for bit in format(k, "b")]
    bits_lsb_first = list(reversed(bits))

    # まだ何も足していないので、楕円曲線の足し算における0から始める。
    # (k == 0 のときは INFINITY (None) を返してください。)
    result = INFINITY

    # 現在処理しているビットに対応する点を保持する。
    # 最初は下位0ビット目なので 2^0 × P = P。
    # ループの最後で毎回2倍するため、この変数は
    #     P → 2P → 4P → 8P → ...
    # と変化する。つまり i を数値として持たなくても、この変数には常に
    # 「現在のビットに対応する 2^i × P」が入っている。
    point_for_current_bit = P

    for bit in bits_lsb_first:
        # 現在のビットが1なら、その桁に対応する 2^i × P を答えに足す。
        if bit == 1:
            result = ec_add(result, point_for_current_bit, curve)

        # 次の（1つ上の）ビットへ進む準備。
        # 現在の 2^i × P を2倍して、次の 2^(i+1) × P を作る。
        point_for_current_bit = ec_add(
            point_for_current_bit,
            point_for_current_bit,
            curve,
        )
...
def sigma_commit(r: int, curve: Curve) -> Point:
    ...
    """
    # [公開] curve.G: 全員が知っている楕円曲線の生成元 G
    # [秘密] r: 証明者だけが知る、今回だけのランダム値（nonce）
    # [公開] R: 検証者へ最初に送る commitment
    R = ec_scalar_mul(r, curve.G, curve)
    return R
...
def sigma_response(x: int, r: int, e: int, curve: Curve) -> int:
    ...
    注意してください。
    """
    # [秘密] x: 秘密鍵（証明したい witness）
    # [秘密] r: commitment R を作るときに使った今回だけのnonce
    # [公開] e: Rを受け取ったあとで検証者が選んだchallenge
    # [公開] s: 検証者へ返すresponse
    s = (r + e * x) % curve.n
    return s
...
def sigma_verify(pubkey: Point, R: Point, e: int, s: int, curve: Curve) -> bool:
    ...
    """
    # 検証者がここで使う値はすべて公開されている。
    # [公開] pubkey = xG: 秘密鍵 x に対応する公開鍵
    # [公開] R = rG: 証明者がchallengeより先に送ったcommitment
    # [公開] e: challenge
    # [公開] s: response

    # 左辺: 証明者から受け取ったresponseを生成元Gへ掛けた点 sG。
    left = ec_scalar_mul(s, curve.G, curve)

    # 右辺: commitment R に、challenge倍した公開鍵 e(xG) を足す。
    e_times_pubkey = ec_scalar_mul(e, pubkey, curve)
    right = ec_add(R, e_times_pubkey, curve)

    # 正直な証明者なら、次の式変形により同じ点になる。
    #   sG = (r + ex)G = rG + e(xG) = R + e*pubkey
    return left == right
...
def schnorr_sign(
    ...
    """
    # [公開] G = curve.G: 楕円曲線の生成元
    # [秘密] x: 署名者の秘密鍵
    # [公開] pubkey = xG: x に対応する公開鍵
    pubkey = ec_scalar_mul(x, curve.G, curve)

    # Sigma Protocolの手番1（Commit）。
    # [秘密] nonce: 対話版での r。署名ごとに新しくする使い捨ての値
    # [公開] R = nonce*G: 署名に含めるcommitment
    R = sigma_commit(nonce, curve)

    # Fiat-Shamir変換。
    # 検証者がランダムに送っていたchallenge eを、R・公開鍵・メッセージ
    # という公開情報のhashで置き換える。検証者も同じeを再計算できる。
    # [公開] e: hashから決まるchallenge
    e = challenge_hash(R, pubkey, message, curve.n)

    # Sigma Protocolの手番3（Response）。
    # [公開] s = nonce + e*x (mod n): 署名に含めるresponse
    s = sigma_response(x, nonce, e, curve)

    # eは公開情報から再計算できるため、署名へ入れる必要がない。
    # xとnonceは秘密のまま、公開する署名は (R, s) の2つだけ。
    return (R, s)
...
def schnorr_verify(
    ...
    """
    # [公開] signature: 署名者が公開した (R, s)
    R, s = signature

    # 署名者と同じ公開情報からchallengeを再計算する。
    # message・R・pubkeyのどれかが変わればeも変わり、元のsでは検証式を
    # 満たせなくなる。これにより署名がこのmessageとpubkeyへ結びつく。
    e = challenge_hash(R, pubkey, message, curve.n)

    # あとは対話版と同じ公開情報だけの検査を行う。
    #   sG == R + e*pubkey
