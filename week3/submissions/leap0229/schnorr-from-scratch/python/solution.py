...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if x_P == x_Q:
        ...
        lam = field_mul(
            field_add(field_mul(3, field_mul(x_P, x_P, p), p), curve.a, p),
            field_inv(field_mul(2, y_P, p), p), p)
    else:
        lam = field_mul(
            sub(y_Q, y_P),
            field_inv(sub(x_Q, x_P), p), p)

    x_R = sub(sub(field_mul(lam, lam, p), x_P), x_Q)
    y_R = sub(field_mul(lam, sub(x_P, x_R), p), y_P)
    return (x_R, y_R)
