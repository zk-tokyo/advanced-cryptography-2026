...
def field_inv(a: int, p: int) -> int:
    ...
    あるので注意してください。
    """
    #aを有限体の範囲に直す。0の時は逆元が存在しないためエラー
    a_mod_p = a % p
    if a_mod_p == 0:
        raise ValueError("逆元が存在しません")

    # a*x + p*y = gcd(a, p) を満たす x を拡張ユークリッド法で求める
    gcd, x, _ = extended_gcd(a_mod_p, p)
    if gcd != 1:
        # pが素数出ない場合は逆元がないためエラー
        raise ValueError("逆元が存在しません")
...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    """
    # P が無限遠点なら Q を、Q が無限遠点なら P を返します
    if P is INFINITY:
        return Q
    if Q is INFINITY:
        return P
    ...
    # x_P == x_Q かつ y_P + y_Q == 0 (mod p) なら INFINITY を返す
    if x_P % p == x_Q % p and (y_P + y_Q) % p == 0:
        return INFINITY

    if P == Q:
        # 同じ点を足すときは、その点における接線の傾きを使う
        numerator = 3 * x_P * x_P + curve.a
        denominator = 2 * y_P
    else:
        # 異なる点を足すときは、2点を結ぶ直線の傾きを使う
        numerator = y_Q - y_P #分子
        denominator = x_Q - x_P #分母
...
def ec_scalar_mul(k: int, P: Point, curve: Curve) -> Point:
    ...
    """
    if k < 0:
        raise ValueError("kが0以下です")

    result = INFINITY       # ここに選んだ2の累乗倍を足していく
    current = P             # 最初は 1*P、次は 2*P、4*P、... と倍にする
    remaining = k

    while remaining > 0:
        remaining, bit = divmod(remaining, 2) #remaining:商 bit:余り
        if bit == 1:
...
def sigma_response(x: int, r: int, e: int, curve: Curve) -> int:
    ...
    注意してください。
    """
    # pは座標の法、nは生成元Gの位数。ここではスカラーなのでnを使う
...
def sigma_verify(pubkey: Point, R: Point, e: int, s: int, curve: Curve) -> bool:
    ...
    e_times_pubkey = ec_scalar_mul(e % curve.n, pubkey, curve)
