...
def ec_add(P: Point, Q: Point, curve: Curve) -> Point:
    ...
    if P == INFINITY:
        ...
        p = curve.p
        if xp == xq and yp == yq:
            xp_2 = field_mul(xp, xp, p)
            lam = field_mul(field_add(field_mul(3, xp_2, p), a, p) ,field_inv(field_mul(2, yp, p), p),p)
        else:
            lam = field_mul((yq - yp) % p, field_inv((xq - xp) % p, p) ,p)

        xr = (field_mul(lam, lam, p) - (xp + xq) % p) % p
        yr = (field_mul(lam, (xp - xr) % p, p) - yp) % p
        return (xr, yr)
...
def ec_scalar_mul(k: int, P: Point, curve: Curve) -> Point:
    ...
    while k != 0:
        if k & 1 == 1:
            result = ec_add(result, base, curve)
        base = ec_add(base, base, curve)
        k >>= 1
    return result
...
def sigma_verify(pubkey: Point, R: Point, e: int, s: int, curve: Curve) -> bool:
    ...
    right = ec_add(R, ec_scalar_mul(e, pubkey, curve), curve)
    return left == right
...
def schnorr_verify(
    ...
    (R,s) = signature
    e = challenge_hash(R, pubkey, message, curve.n)
    return ec_scalar_mul(s, curve.G, curve) == ec_add(R, ec_scalar_mul(e, pubkey, curve), curve)
